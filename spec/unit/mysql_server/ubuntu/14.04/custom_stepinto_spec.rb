...
describe 'stepped into mysql_test_custom::server on ubuntu-14.04' do
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mysql_service and creates template[/var/cache/local/preseeding/mysql-server.seed]' do
      expect(ubuntu_14_04_custom_run).to create_template('/var/cache/local/preseeding/mysql-server.seed').with(
        :cookbook => 'mysql',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mysql_service and creates execute[preseed mysql-server]' do
      expect(ubuntu_14_04_custom_run).to_not run_execute('preseed mysql-server').with(
    ...
    it 'steps into mysql_service and creates service[apparmor-mysql]' do
      expect(ubuntu_14_04_custom_run).to_not start_service('apparmor-mysql')
    end
    ...
    it 'steps into mysql_service and renders file[/etc/mysql/my.cnf]' do
      expect(ubuntu_14_04_custom_run).to render_file('/etc/mysql/my.cnf').with_content(my_cnf_5_5_content_ubuntu_14_04)
    end

    it 'steps into mysql_service and creates bash[move mysql data to datadir]' do
      expect(ubuntu_14_04_custom_run).to_not run_bash('move mysql data to datadir')
    end
