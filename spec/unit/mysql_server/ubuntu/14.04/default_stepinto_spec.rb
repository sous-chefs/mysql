...
describe 'stepped into mysql_test_default::server on ubuntu-14.04' do
...
SET PASSWORD FOR 'root'@'127.0.0.1' = PASSWORD('ilikerandompasswords');"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mysql_service and creates template[/var/cache/local/preseeding/mysql-server.seed]' do
      expect(ubuntu_14_04_default_run).to create_template('/var/cache/local/preseeding/mysql-server.seed').with(
        :cookbook => 'mysql',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mysql_service and creates execute[preseed mysql-server]' do
      expect(ubuntu_14_04_default_run).to_not run_execute('preseed mysql-server').with(
    ...
    it 'steps into mysql_service and installs package[mysql-server-5.5]' do
      expect(ubuntu_14_04_default_run).to install_package('mysql-server-5.5')
    end
    ...
    it 'steps into mysql_service and creates service[apparmor-mysql]' do
      expect(ubuntu_14_04_default_run).to_not start_service('apparmor-mysql')
    end
    ...
    it 'steps into mysql_service and creates bash[move mysql data to datadir]' do
      expect(ubuntu_14_04_default_run).to_not run_bash('move mysql data to datadir')
    end
  end
end
