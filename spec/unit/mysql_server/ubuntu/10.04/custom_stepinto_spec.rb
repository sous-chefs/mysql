...
describe 'stepped into mysql_test_custom::server on ubuntu-10.04' do
...
GRANT ALL PRIVILEGES ON *.* TO 'root'@'1.2.3.4/5' IDENTIFIED BY 'YUNOSETPASSWORD' WITH GRANT OPTION;"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mysql_service and creates template[/var/cache/local/preseeding/mysql-server.seed]' do
      expect(ubuntu_10_04_custom_run).to create_template('/var/cache/local/preseeding/mysql-server.seed').with(
        :cookbook => 'mysql',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mysql_service and creates execute[preseed mysql-server]' do
      expect(ubuntu_10_04_custom_run).to_not run_execute('preseed mysql-server').with(
    ...
    it 'steps into mysql_service and creates template[/etc/mysql/debian.cnf]' do
      expect(ubuntu_10_04_custom_run).to create_template('/etc/mysql/debian.cnf').with(
        :cookbook => 'mysql',
    ...
    it 'steps into mysql_service and creates template[/etc/apparmor.d/usr.sbin.mysqld]' do
      expect(ubuntu_10_04_custom_run).to create_template('/etc/apparmor.d/usr.sbin.mysqld').with(
        :cookbook => 'mysql',
        :owner => 'root',
        :group => 'root',
        :mode => '0644'
    ...
    it 'steps into mysql_service and creates service[apparmor-mysql]' do
      expect(ubuntu_10_04_custom_run).to_not start_service('apparmor-mysql')
    end

    it 'steps into mysql_service and creates service[mysql]' do
      expect(ubuntu_10_04_custom_run).to start_service('mysql')
      expect(ubuntu_10_04_custom_run).to enable_service('mysql')
    end

    it 'steps into mysql_service and creates directory[/etc/mysql/conf.d]' do
      expect(ubuntu_10_04_custom_run).to create_directory('/etc/mysql/conf.d').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mysql_service and creates directory[/var/run/mysqld]' do
      expect(ubuntu_10_04_custom_run).to create_directory('/var/run/mysqld').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mysql_service and creates template[/etc/mysql_grants.sql]' do
      expect(ubuntu_10_04_custom_run).to create_template('/etc/mysql_grants.sql').with(
        :cookbook => 'mysql',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mysql_service and renders file[/etc/mysql_grants.sql]' do
      expect(ubuntu_10_04_custom_run).to render_file('/etc/mysql_grants.sql').with_content(
    ...
    it 'steps into mysql_service and creates template[/etc/mysql/my.cnf]' do
      expect(ubuntu_10_04_custom_run).to create_template('/etc/mysql/my.cnf').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0600'
        )
    ...
    it 'steps into mysql_service and renders file[/etc/mysql/my.cnf]' do
      expect(ubuntu_10_04_custom_run).to render_file('/etc/mysql/my.cnf').with_content(
        my_cnf_5_5_content_custom_ubuntu_10_04
        )
    end

    it 'steps into mysql_service and creates bash[move mysql data to datadir]' do
      expect(ubuntu_10_04_custom_run).to_not run_bash('move mysql data to datadir')
    end
