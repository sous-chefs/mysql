...
describe 'stepped into mysql_test_custom::server on ubuntu-13.10' do
  ...
  context 'when using default parameters' do
    it 'creates mysql_service[ubuntu_13_10_default]' do
      expect(ubuntu_13_10_custom_run).to create_mysql_service('ubuntu_13_10_default').with(
        :version => '5.5',
        :port => '3308',
        :data_dir => '/data'
        )
    ...
    it 'steps into mysql_service and installs package[debconf-utils]' do
      expect(ubuntu_13_10_custom_run).to install_package('debconf-utils')
    end

    it 'steps into mysql_service and creates directory[/var/cache/local/preseeding]' do
      expect(ubuntu_13_10_custom_run).to create_directory('/var/cache/local/preseeding').with(
        :owner => 'root',
        :group => 'root',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mysql_service and creates execute[preseed mysql-server]' do
      expect(ubuntu_13_10_custom_run).to_not run_execute('preseed mysql-server').with(
    ...
    it 'steps into mysql_service and installs package[mysql-server]' do
      expect(ubuntu_13_10_custom_run).to install_package('mysql-server')
    end

    # apparmor
    it 'steps into mysql_service and creates directory[/etc/apparmor.d]' do
      expect(ubuntu_13_10_custom_run).to create_directory('/etc/apparmor.d').with(
        :owner => 'root',
        :group => 'root',
        :mode => '0755'
        )
    ...
    it 'steps into mysql_service and creates template[/etc/mysql/debian.cnf]' do
      expect(ubuntu_13_10_custom_run).to create_template('/etc/mysql/debian.cnf').with(
        :cookbook => 'mysql',
    ...
    it 'steps into mysql_service and creates service[apparmor-mysql]' do
      expect(ubuntu_13_10_custom_run).to_not start_service('apparmor-mysql')
    end

    it 'steps into mysql_service and creates service[mysql]' do
      expect(ubuntu_13_10_custom_run).to start_service('mysql')
      expect(ubuntu_13_10_custom_run).to enable_service('mysql')
    end

    it 'steps into mysql_service and creates directory[/etc/mysql/conf.d]' do
      expect(ubuntu_13_10_custom_run).to create_directory('/etc/mysql/conf.d').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mysql_service and creates directory[/data]' do
      expect(ubuntu_13_10_custom_run).to create_directory('/data').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mysql_service and creates execute[assign-root-password]' do
      expect(ubuntu_13_10_custom_run).to run_execute('assign-root-password').with(
    ...
    it 'steps into mysql_service and creates execute[install-grants]' do
      expect(ubuntu_13_10_custom_run).to_not run_execute('install-grants').with(
    ...
    it 'steps into mysql_service and creates template[/etc/mysql/my.cnf]' do
      expect(ubuntu_13_10_custom_run).to create_template('/etc/mysql/my.cnf').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0600'
        )
    ...
    it 'steps into mysql_service and renders file[/etc/mysql/my.cnf]' do
      expect(ubuntu_13_10_custom_run).to render_file('/etc/mysql/my.cnf').with_content(my_cnf_5_5_content_ubuntu_13_10)
    end

    it 'steps into mysql_service and creates bash[move mysql data to datadir]' do
      expect(ubuntu_13_10_custom_run).to_not run_bash('move mysql data to datadir')
    end

    it 'steps into mysql_service and writes log[notify restart]' do
      expect(ubuntu_13_10_custom_run).to write_log('notify restart')
    end

    it 'steps into mysql_service and writes log[notify reload]' do
      expect(ubuntu_13_10_custom_run).to write_log('notify reload')
    end
  end
end
