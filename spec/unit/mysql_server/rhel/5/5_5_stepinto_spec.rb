...
describe 'stepped into mysql_test_custom::server 5.5 on centos-5.8' do
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mysql_service and installs package[mysql55-mysql-server]' do
      expect(centos_5_8_default_run).to install_package('mysql55-mysql-server')
    end

    it 'steps into mysql_service and creates directory[/opt/rh/mysql55/root/etc/mysql/conf.d]' do
      expect(centos_5_8_default_run).to create_directory('/opt/rh/mysql55/root/etc/mysql/conf.d').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mysql_service and creates directory[/opt/rh/mysql55/root/var/run/mysqld/]' do
      expect(centos_5_8_default_run).to create_directory('/opt/rh/mysql55/root/var/run/mysqld/').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mysql_service and creates template[/opt/rh/mysql55/root/etc/my.cnf]' do
      expect(centos_5_8_default_run).to create_template('/opt/rh/mysql55/root/etc/my.cnf').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0600'
        )
    ...
    it 'steps into mysql_service and creates service[mysql55-mysqld]' do
      expect(centos_5_8_default_run).to start_service('mysql55-mysqld')
      expect(centos_5_8_default_run).to enable_service('mysql55-mysqld')
    end
    ...
    it 'steps into mysql_service and renders file[/opt/rh/mysql55/root/etc/my.cnf]' do
      expect(centos_5_8_default_run).to render_file('/opt/rh/mysql55/root/etc/my.cnf').with_content(my_cnf_5_5_content_centos_5_8)
    end
