...
describe 'stepped into mysql_test_custom::server 5.0 on centos-5.8' do
  let(:centos_5_8_custom_run) do
    ChefSpec::Runner.new(
      ...
      node.set['mysql']['service_name'] = 'centos_5_8_custom_stepinto'
  ...
  context 'when using default parameters' do
    it 'creates mysql_service[centos_5_8_custom_stepinto]' do
      expect(centos_5_8_custom_run).to create_mysql_service('centos_5_8_custom_stepinto').with(
        :version => '5.0',
        :port => '3308',
        :data_dir => '/data'
        )
    ...
    it 'steps into mysql_service and renders file[/etc/my.cnf]' do
      expect(centos_5_8_custom_run).to render_file('/etc/my.cnf').with_content(my_cnf_5_5_content_centos_5_8)
    end
