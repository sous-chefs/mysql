...
describe 'stepped into mysql_test_custom::server 5.1 on centos-5.8' do
  ...
  before do
    stub_command("/opt/rh/mysql51/root/usr/bin/mysql -u root -e 'show databases;'").and_return(true)
  end
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mysql_service and installs package[mysql51-mysql-server]' do
      expect(centos_5_8_default_run).to install_package('mysql51-mysql-server')
    end

    it 'steps into mysql_service and creates directory[/opt/rh/mysql51/root/etc/mysql/conf.d]' do
      expect(centos_5_8_default_run).to create_directory('/opt/rh/mysql51/root/etc/mysql/conf.d').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mysql_service and creates directory[/opt/rh/mysql51/root/var/run/mysqld/]' do
      expect(centos_5_8_default_run).to create_directory('/opt/rh/mysql51/root/var/run/mysqld/').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mysql_service and creates template[/opt/rh/mysql51/root/etc/my.cnf]' do
      expect(centos_5_8_default_run).to create_template('/opt/rh/mysql51/root/etc/my.cnf').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0600'
        )
    ...
    it 'steps into mysql_service and creates service[mysql51-mysqld]' do
      expect(centos_5_8_default_run).to start_service('mysql51-mysqld')
      expect(centos_5_8_default_run).to enable_service('mysql51-mysqld')
    end
    ...
    it 'steps into mysql_service and renders file[/opt/rh/mysql51/root/etc/my.cnf]' do
      expect(centos_5_8_default_run).to render_file('/opt/rh/mysql51/root/etc/my.cnf').with_content(my_cnf_5_5_content_centos_5_8)
    end
