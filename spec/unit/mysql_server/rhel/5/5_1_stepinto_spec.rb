...
GRANT ALL PRIVILEGES ON *.* TO 'root'@'1.2.3.4/5' IDENTIFIED BY 'YUNOSETPASSWORD' WITH GRANT OPTION;"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mysql_service and installs package[mysql51-mysql-server]' do
      expect(centos_5_8_custom2_run).to install_package('mysql51-mysql-server')
    end

    it 'steps into mysql_service and creates directory[/opt/rh/mysql51/root/etc/mysql/conf.d]' do
      expect(centos_5_8_custom2_run).to create_directory('/opt/rh/mysql51/root/etc/mysql/conf.d').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0750',
        :recursive => true
    ...
    it 'steps into mysql_service and creates directory[/opt/rh/mysql51/root/var/run/mysqld/]' do
      expect(centos_5_8_custom2_run).to create_directory('/opt/rh/mysql51/root/var/run/mysqld/').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0755',
        :recursive => true
    ...
    it 'steps into mysql_service and creates template[/opt/rh/mysql51/root/etc/my.cnf]' do
      expect(centos_5_8_custom2_run).to create_template('/opt/rh/mysql51/root/etc/my.cnf').with(
        :owner => 'mysql',
        :group => 'mysql',
        :mode => '0600'
        )
    ...
    it 'steps into mysql_service and creates template[/etc/mysql_grants.sql]' do
      expect(centos_5_8_custom2_run).to create_template('/etc/mysql_grants.sql').with(
        :cookbook => 'mysql',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mysql_service and creates bash[move mysql data to datadir]' do
      expect(centos_5_8_custom2_run).to_not run_bash('move mysql data to datadir')
    end
