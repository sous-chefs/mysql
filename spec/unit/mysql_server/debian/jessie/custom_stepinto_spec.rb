...
describe 'stepped into mysql_test_custom::server on debian-jessie' do
  let(:debian_jessie_custom_run) do
    ChefSpec::Runner.new(
      ...
      node.set['mysql']['service_name'] = 'debian_jessie_custom'
...
GRANT ALL PRIVILEGES ON *.* TO 'root'@'1.2.3.4/5' IDENTIFIED BY 'YUNOSETPASSWORD' WITH GRANT OPTION;"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mysql_service and creates template[/var/cache/local/preseeding/mysql-server.seed]' do
      expect(debian_jessie_custom_run).to create_template('/var/cache/local/preseeding/mysql-server.seed').with(
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
        )
    ...
    it 'steps into mysql_service and creates execute[preseed mysql-server]' do
      expect(debian_jessie_custom_run).to_not run_execute('preseed mysql-server').with(
    ...
    it 'steps into mysql_service and creates bash[move mysql data to datadir]' do
      expect(debian_jessie_custom_run).to_not run_bash('move mysql data to datadir')
    end
