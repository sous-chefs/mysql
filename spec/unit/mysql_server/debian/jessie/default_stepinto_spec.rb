...
SET PASSWORD FOR 'root'@'127.0.0.1' = PASSWORD('ilikerandompasswords');"
  ...
  context 'when using default parameters' do
    ...
    it 'steps into mysql_service and creates template[/var/cache/local/preseeding/mysql-server.seed]' do
      expect(debian_jessie_default_run).to create_template('/var/cache/local/preseeding/mysql-server.seed').with(
        :cookbook => 'mysql',
        :owner => 'root',
        :group => 'root',
        :mode => '0600'
    ...
    it 'steps into mysql_service and creates execute[preseed mysql-server]' do
      expect(debian_jessie_default_run).to_not run_execute('preseed mysql-server').with(
    ...
    it 'steps into mysql_service and installs package[mysql-server-5.5]' do
      expect(debian_jessie_default_run).to install_package('mysql-server-5.5')
    end
    ...
    it 'steps into mysql_service and creates bash[move mysql data to datadir]' do
      expect(debian_jessie_default_run).to_not run_bash('move mysql data to datadir')
    end
  end
end
