...
describe 'mysql_service_test::multi on omnios-151014' do
  cached(:omnios_service_56_multi) do
  ...
  context 'compiling the test recipe' do
    it 'creates user[alice]' do
      expect(omnios_service_56_multi).to create_user('alice')
    end

    it 'creates group[alice]' do
      expect(omnios_service_56_multi).to create_group('alice')
    end

    it 'creates user[bob]' do
      expect(omnios_service_56_multi).to create_user('bob')
    end

    it 'creates group[bob]' do
      expect(omnios_service_56_multi).to create_group('bob')
    end

    it 'deletes mysql_service[default]' do
      expect(omnios_service_56_multi).to delete_mysql_service('default')
    end

    it 'creates mysql_service[instance-1]' do
      expect(omnios_service_56_multi).to create_mysql_service('instance-1')
    end

    it 'creates mysql_service[instance-2]' do
      expect(omnios_service_56_multi).to create_mysql_service('instance-2')
    end
  end
  ...
  context 'stepping into mysql_service[default] resource' do
    it 'stops service[default :delete mysql-default]' do
      expect(omnios_service_56_multi).to stop_service('default :delete mysql-default')
        .with(
    ...
    it 'deletes directory[default :delete /opt/mysql55/etc/mysql-default]' do
      expect(omnios_service_56_multi).to delete_directory('default :delete /opt/mysql55/etc/mysql-default')
        .with(
    ...
    it 'deletes directory[default :delete /var/run/mysql-default]' do
      expect(omnios_service_56_multi).to delete_directory('default :delete /var/run/mysql-default')
        .with(
    ...
    it 'deletes directory[default :delete /var/adm/log/mysql-default]' do
      expect(omnios_service_56_multi).to delete_directory('default :delete /var/adm/log/mysql-default')
        .with(
  ...
  context 'stepping into mysql_service[instance-1] resource' do
    it 'installs package[instance-1 :create database/mysql-56]' do
      expect(omnios_service_56_multi).to install_package('instance-1 :create database/mysql-56')
        .with(package_name: 'database/mysql-56', version: nil)
    end

    it 'starts service[instance-1 :start mysql-instance-1]' do
      expect(omnios_service_56_multi).to enable_service('instance-1 :start mysql-instance-1')
        .with(
    ...
    it 'creates group[instance-1 :create mysql]' do
      expect(omnios_service_56_multi).to create_group('instance-1 :create mysql')
        .with(group_name: 'mysql')
    end

    it 'creates user[instance-1 :create mysql]' do
      expect(omnios_service_56_multi).to create_user('instance-1 :create mysql')
        .with(username: 'mysql')
    end

    it 'deletes file[instance-1 :create /opt/mysql56/etc/mysql/my.cnf]' do
      expect(omnios_service_56_multi).to delete_file('instance-1 :create /opt/mysql56/etc/mysql/my.cnf')
        .with(path: '/opt/mysql56/etc/mysql/my.cnf')
    end

    it 'deletes file[instance-1 :create /opt/mysql56/etc/my.cnf]' do
      expect(omnios_service_56_multi).to delete_file('instance-1 :create /opt/mysql56/etc/my.cnf')
        .with(path: '/opt/mysql56/etc/my.cnf')
    end

    it 'creates link[instance-1 :create /opt/mysql56/usr/share/my-default.cnf]' do
      expect(omnios_service_56_multi).to create_link('instance-1 :create /opt/mysql56/usr/share/my-default.cnf')
        .with(
    ...
    it 'creates directory[instance-1 :create /opt/mysql56/etc/mysql-instance-1]' do
      expect(omnios_service_56_multi).to create_directory('instance-1 :create /opt/mysql56/etc/mysql-instance-1')
        .with(
    ...
    it 'creates directory[instance-1 :create /opt/mysql56/etc/mysql-instance-1/conf.d]' do
      expect(omnios_service_56_multi).to create_directory('instance-1 :create /opt/mysql56/etc/mysql-instance-1/conf.d')
        .with(
    ...
    it 'creates directory[instance-1 :create /var/run/mysql-instance-1]' do
      expect(omnios_service_56_multi).to create_directory('instance-1 :create /var/run/mysql-instance-1')
        .with(
    ...
    it 'creates directory[instance-1 :create /var/adm/log/mysql-instance-1]' do
      expect(omnios_service_56_multi).to create_directory('instance-1 :create /var/adm/log/mysql-instance-1')
        .with(
    ...
    it 'creates directory[instance-1 :create /data/instance-1]' do
      expect(omnios_service_56_multi).to create_directory('instance-1 :create /data/instance-1')
        .with(
    ...
    it 'creates template[instance-1 :create /opt/mysql56/etc/mysql-instance-1/my.cnf]' do
      expect(omnios_service_56_multi).to create_template('instance-1 :create /opt/mysql56/etc/mysql-instance-1/my.cnf')
        .with(
    ...
    it 'runs bash[instance-1 :create initialize mysql database]' do
      expect(omnios_service_56_multi).to_not run_bash('instance-1 :create initialize mysql database')
        .with(
    ...
    it 'runs bash[instance-1 :create initial records]' do
      expect(omnios_service_56_multi).to_not run_bash('instance-1 :create initial records')
    end

    it 'create template[instance-1 :start /lib/svc/method/mysql-instance-1]' do
      expect(omnios_service_56_multi).to create_template('instance-1 :start /lib/svc/method/mysql-instance-1')
        .with(
    ...
    it 'create template[instance-1 :start /lib/svc/method/mysql-instance-1]' do
      expect(omnios_service_56_multi).to install_smf('mysql-instance-1')
        .with(
  ...
  context 'stepping into mysql_service[instance-2] resource' do
    it 'installs package[instance-2 :create database/mysql-56]' do
      expect(omnios_service_56_multi).to install_package('instance-2 :create database/mysql-56')
        .with(package_name: 'database/mysql-56', version: nil)
    end

    it 'creates group[instance-2 :create mysql]' do
      expect(omnios_service_56_multi).to create_group('instance-2 :create mysql')
        .with(group_name: 'mysql')
    end

    it 'creates user[instance-2 :create mysql]' do
      expect(omnios_service_56_multi).to create_user('instance-2 :create mysql')
        .with(username: 'mysql')
    end

    it 'deletes file[instance-2 :create /opt/mysql56/etc/mysql/my.cnf]' do
      expect(omnios_service_56_multi).to delete_file('instance-2 :create /opt/mysql56/etc/mysql/my.cnf')
        .with(path: '/opt/mysql56/etc/mysql/my.cnf')
    end

    it 'deletes file[instance-2 :create /opt/mysql56/etc/my.cnf]' do
      expect(omnios_service_56_multi).to delete_file('instance-2 :create /opt/mysql56/etc/my.cnf')
        .with(path: '/opt/mysql56/etc/my.cnf')
    end

    it 'creates link[instance-2 :create /opt/mysql56/usr/share/my-default.cnf]' do
      expect(omnios_service_56_multi).to create_link('instance-2 :create /opt/mysql56/usr/share/my-default.cnf')
        .with(
          target_file: '/opt/mysql56/usr/share/my-default.cnf',
    ...
    it 'creates directory[instance-2 :create /opt/mysql56/etc/mysql-instance-2]' do
      expect(omnios_service_56_multi).to create_directory('instance-2 :create /opt/mysql56/etc/mysql-instance-2')
        .with(
    ...
    it 'creates directory[instance-2 :create /opt/mysql56/etc/mysql-instance-2/conf.d]' do
      expect(omnios_service_56_multi).to create_directory('instance-2 :create /opt/mysql56/etc/mysql-instance-2/conf.d')
        .with(
    ...
    it 'creates directory[instance-2 :create /var/run/mysql-instance-2]' do
      expect(omnios_service_56_multi).to create_directory('instance-2 :create /var/run/mysql-instance-2')
        .with(
    ...
    it 'creates directory[instance-2 :create /var/adm/log/mysql-instance-2]' do
      expect(omnios_service_56_multi).to create_directory('instance-2 :create /var/adm/log/mysql-instance-2')
        .with(
    ...
    it 'creates directory[instance-2 :create /data/instance-2]' do
      expect(omnios_service_56_multi).to create_directory('instance-2 :create /data/instance-2')
        .with(
    ...
    it 'creates template[instance-2 :create /opt/mysql56/etc/mysql-instance-2/my.cnf]' do
      expect(omnios_service_56_multi).to create_template('instance-2 :create /opt/mysql56/etc/mysql-instance-2/my.cnf')
        .with(
    ...
    it 'runs bash[instance-2 :create initialize mysql database]' do
      expect(omnios_service_56_multi).to_not run_bash('instance-2 :create initialize mysql database')
        .with(
    ...
    it 'runs bash[instance-2 :create initial records]' do
      expect(omnios_service_56_multi).to_not run_bash('instance-2 :create initial records')
    end

    it 'create template[instance-2 :start /lib/svc/method/mysql-instance-2]' do
      expect(omnios_service_56_multi).to create_template('instance-2 :start /lib/svc/method/mysql-instance-2')
        .with(
    ...
    it 'create template[instance-2 :start /lib/svc/method/mysql-instance-2]' do
      expect(omnios_service_56_multi).to install_smf('mysql-instance-2')
        .with(
    ...
    it 'starts service[instance-2 :start mysql-instance-2]' do
      expect(omnios_service_56_multi).to enable_service('instance-2 :start mysql-instance-2')
        .with(
