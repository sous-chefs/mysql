...
template "#{base_dir}/etc/my.cnf" do
  source 'omnios-r151006c/5.6/my.cnf.erb'
  owner 'mysql'
  mode '0600'
  variables(
    :base_dir => base_dir,
...
template '/lib/svc/method/mysqld' do
  source 'omnios-r151006c/5.6/svc.method.mysqld.erb'
  mode '0555'
  variables(
    :base_dir => base_dir,
    :data_dir => node['mysql']['data_dir'],
...
template '/tmp/mysql.xml' do
  source 'omnios-r151006c/5.6/mysql.xml.erb'
  owner 'root'
  mode '0644'
  action :create
