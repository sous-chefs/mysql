...
template '/var/cache/local/preseeding/mysql-server.seed' do
  source 'ubuntu-12.04/5.5/mysql-server.seed.erb'
  owner 'root'
  group 'root'
  mode '0600'
...
template '/etc/mysql/debian.cnf' do
  source 'ubuntu-12.04/5.5/debian.cnf.erb'
  owner 'root'
  group 'root'
  mode '0600'
end
...
template '/etc/apparmor.d/usr.sbin.mysqld' do
  source 'ubuntu-12.04/5.5/usr.sbin.mysqld.erb'
  action :create
...
template '/etc/mysql/my.cnf' do
  source 'ubuntu-12.04/5.5/my.cnf.erb'
  owner 'root'
  group 'root'
  mode '0644'
  variables(
