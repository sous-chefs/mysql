  ...
  class Provider
    class MysqlServiceSystemd < Chef::Provider::MysqlServiceBase
      ...
      action :start do
        ...
        template "#{new_resource.name} :start #{systemd_unit_path}/#{mysql_name}.service" do
          path "#{systemd_unit_path}/#{mysql_name}.service"
      ...
      action :stop do
        ...
        service "#{new_resource.name} :stop #{mysql_name}" do
          ...
          action [:disable, :stop]
          only_if { ::File.exist?("#{systemd_unit_path}/#{mysql_name}.service") }
        end
      end
